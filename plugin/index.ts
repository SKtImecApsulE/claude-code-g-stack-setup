/**
 * Trust Layer v0.2 — the custom core of the personal assistant.
 *
 *   agent tool intent
 *        │
 *        ▼
 ...
 *        │   │                                                 ├─ auto      → execute*
 *        │   │  *content-bearing session (an email body was    ├─ confirm   → requireApproval
 *        │   │   read this session) downgrades auto → confirm  └─ escalate/unknown → block
 *        │   └─ requireApproval.onResolution: deny → streak reset
 *        ▼
 *   tool execute ──► ledger (SQLite: actions + task_types + spend, one DB)
 *        │                ├─ streak++ on executed confirm-tier action
 *        │                └─ streak ≥ 10 → promotion pitch appended to result
 *        ├─ trust_ledger   (inspect)      ├─ trust_promote (gated by approval)
 *        ├─ trust_policy   (tiers/streaks/spend)  ├─ trust_revoke (always allowed)
 *        └─ trust_undo     (LIFO inverse, precondition-checked, itself ledgered)
 *
 * v0 scope: mock Gmail/Calendar tools on a JSON mailbox — drivable end-to-end
 * without Google credentials. Classification is deterministic (tool + args);
 * the LLM never self-reports a task type; unknown → escalate.
 */
...
const DATA_DIR = process.env.PA_DATA_DIR || join(homedir(), ".personal-assistant");
...
type Email = { id: string; from: string; subject: string; body: string; archived: boolean };
...
// ── trust DB: one SQLite file — task_types + actions + spend (decision 3A) ──
...
const insType = db.prepare("INSERT OR IGNORE INTO task_types (name, tier) VALUES (?, ?)");
...
const BUDGET_WARN_USD = 2;
const BUDGET_THROTTLE_USD = 5;
...
export function classify(tool: string, params: Record<string, unknown>): string | null {
...
function ledgerAppend(e: {
  ...
  db.prepare(
    ...
    e.tier,
    e.reversible ? 1 : 0,
    e.inverse ? JSON.stringify(e.inverse) : null,
    e.resultSummary
  );
}

const GATED_TOOLS = new Set(["gmail_archive", "gmail_create_draft", "gmail_send_draft", "calendar_hold"]);
// Reading an email body puts untrusted content in the session's context.
const CONTENT_READ_TOOLS = new Set(["gmail_read_email"]);
// Sessions that have read untrusted content this process lifetime (2A patched:
// provenance is session-level; content-bearing sessions never run at auto).
const contentSessions = new Set<string>();

function todaySpend(): { day: string; calls: number; tokens: number; usd: number } {
  const day = new Date().toISOString().slice(0, 10);
  const row = db.prepare("SELECT * FROM spend WHERE day = ?").get(day) as any;
...
export default {
  ...
  register(api: any) {
    // ── the tier gate (decision 1A: OpenClaw owns approval delivery) ──
    api.on("before_tool_call", (event: any, ctx: any) => {
      const sessionKey = ctx?.sessionKey ?? "unknown-session";
      if (CONTENT_READ_TOOLS.has(event.toolName)) {
        contentSessions.add(sessionKey); // mark BEFORE the read executes
        return;
      }
      // Promoting a task type expands the agent's power — always needs your ✅.
      ...
      if (!GATED_TOOLS.has(event.toolName)) return; // trust_*/read tools pass
      const taskType = classify(event.toolName, event.params ?? {});
      let tier = tierOf(taskType);
      let provenanceNote = "";
      if (tier === "auto" && contentSessions.has(sessionKey)) {
        tier = "confirm"; // 2A: content-bearing session — nothing runs at auto
        ...
        return {
          requireApproval: {
            ...
            onResolution: (decision: string) => {
              // A rejection is a trust event: reset the streak (design rule).
    ...
    // ── budget: meter every model call into the spend table ──
    api.on("model_call_ended", (event: any) => {
      ...
      db.prepare(
        `INSERT INTO spend (day, calls, tokens, usd) VALUES (?, 1, ?, ?)
         ON CONFLICT(day) DO UPDATE SET calls = calls + 1, tokens = tokens + excluded.tokens, usd = usd + excluded.usd`
      ).run(day, tokens, usd);
    });

    // ── ledger writes live inside execution (hooks proved skippable) ──
    const ledgerExec = (
      tool: string,
      params: Record<string, unknown>,
      resultSummary: string,
      ...
      if (taskType && tier === "confirm") {
        ...
        const s = (db.prepare("SELECT streak FROM task_types WHERE name = ?").get(taskType) as any).streak;
    ...
    api.registerTool({
      ...
      async execute() {
        ...
        return text(
          mb.emails.filter((e) => !e.archived).map((e) => `${e.id} | from: ${e.from} | ${e.subject}`).join("\n") ||
            "(inbox empty)"
        );
      },
    });
    ...
    // ── promotion machinery (the receipts) ──
    api.registerTool({
      name: "trust_promote",
      ...
      async execute(_id: string, params: any) {
        const row = db.prepare("SELECT * FROM task_types WHERE name = ?").get(params.task_type) as any;
        if (!row) return text(`unknown task type: ${params.task_type}`);
        if (row.tier !== "confirm") return text(`'${row.name}' is tier ${row.tier} — only confirm-tier types can be promoted`);
        if (row.streak < PROMOTION_STREAK) return text(`'${row.name}' has streak ${row.streak}/${PROMOTION_STREAK} — not yet earned. The record has to come first.`);
    ...
    api.registerTool({
      ...
      async execute(_id: string, params: any) {
        const rows = db.prepare("SELECT * FROM actions ORDER BY id DESC LIMIT ?").all(Math.min(params?.limit ?? 10, 50)) as any[];
          ...
          rows
            .map(
              (r) =>
                ...
                `${r.reversible ? "reversible" : "IRREVERSIBLE"}${r.undone ? " (undone)" : ""}${r.undo_failed ? " (undo FAILED)" : ""} — ${r.result_summary}`
            )
            .join("\n")
        );
      },
    ...
    api.registerTool({
      ...
      async execute() {
        ...
        const s = todaySpend();
        return text(
          rows.map((r) => `${r.name}: tier=${r.tier} streak=${r.streak}`).join("\n") +
    ...
    api.registerTool({
      ...
      async execute(_id: string, params: any) {
        ...
        for (let i = 0; i < n; i++) {
          const row = db
            .prepare("SELECT * FROM actions WHERE undone = 0 AND undo_failed = 0 AND tool NOT IN ('trust_undo','trust_revoke') ORDER BY id DESC LIMIT 1")
            .get() as any;
          if (!row) { out.push("nothing left to undo"); break; }
          if (!row.reversible) { out.push(`#${row.id} ${row.tool} is IRREVERSIBLE — halting (by design)`); break; }
          ...
          if (inverse?.op === "unarchive") {
            ...
            ok = true; msg = `demoted ${inverse.task_type} back to confirm`;
          } else {
